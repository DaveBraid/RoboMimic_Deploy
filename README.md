...
#### 2.2.3 安装 unitree_cpp

安装unitree_cpp是一个使用pybind，将 CPP 版本 Unitree SDK2 封装为 Python Interface 的库。使用 pyproject.toml 进行 cmake 编译。

用它可以方便地将存在性能问题的 unitree python sdk2 给替换为 Cpp版本，进而获得更好的实物部署效果。（特别是替换后可以修复 Loco Mode 抖动的问题）

作者已 fork 原仓库。安装步骤如下：


```bash
# cd RoboMimic_Deploy  # assume you are in this folder
git clone https://github.com/DaveBraid/unitree_cpp.git
cd unitree_cpp
pip install .
...
2. 运行deploy_real程序：
```bash
python deploy_real/deploy_real_cpp_sdk2.py
```
3. Start键进入位控模式
...
**使用 unitree_cpp 的本文已克服该问题。请配合使用 `deploy_real/deploy_real_cpp_sdk2.py` 使用**
