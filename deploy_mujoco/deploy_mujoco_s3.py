    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and Running:
            try:
                ...
                tau = pd_control(policy_output_action, d.qpos[7:], kps, np.zeros_like(kds), d.qvel[6:], kds)
                d.ctrl[:] = tau
                mujoco.mj_step(m, d)
                sim_counter += 1
                if sim_counter % control_decimation == 0:
                    
                    qj = d.qpos[7:]  # 广义位置，前7个是free joint浮动基座的位置(3)和姿态(4,四元数)
                    dqj = d.qvel[6:]  # 广义速度，前6个是浮动基座的线速度(3)和角速度(3)
                    quat = d.qpos[3:7]  # 浮动基座的姿态四元数

                    omega = d.qvel[3:6]  # 浮动基座的角速度
